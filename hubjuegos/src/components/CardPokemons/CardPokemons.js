...
export const CardsPokemons = (data) => {
     ...
     * ternario en el que en caso de incluir el id de este pokemon en el array de fav del usuario del contexto
    ...
    data.map((pokemon) => {
      ...
      <h2>${pokemon.name}</h2>
      <span class="material-symbols-outlined  ${appUser.fav.includes(pokemon.id.toString()) ? "like" : ""
            }"> favorite </span>
    </figure>`;
...
const addListeners = (data) => {
    ...
    spanAll.forEach((span) => {
        span.addEventListener("click", (e) => {
             ...
             *
             *
             * Esto lo va a saber gracias al usuario que hay en el contexto y su array de fav ya que lo que hace en
            ...
            if (appUser.fav.includes(e.target.parentNode.id)) {
                ...
                /** importante a ese span quitarle la clase like para asi se le quite el rojo */
                span.classList.toggle("like");
            } else {
                /** En caso de no incluir ese id en el arrray de fav del user logado del contesto lo que tenemos que hacer es meter el id de ese figure
